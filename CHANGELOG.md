...
### Fixed
- Cyrus no longer reacts to messages it wasn't addressed in. In any chat channel/group where the bot is present (Feishu or Slack), plain messages that don't @mention Cyrus — and aren't part of a conversation Cyrus is already in — used to get an "OnIt" emoji receipt anyway, even though Cyrus then correctly stayed silent. Now Cyrus adds no reaction and takes no action on those messages; it only reacts when you @mention it or when you continue a thread it's already working in. ([IN-13](https://linear.app/principle-intl/issue/IN-13))
